#!/usr/bin/env bash
#
# Runs the db-heavy rate ladder against whatever variant is currently running on the
# SUT. Statement query padded to a 15ms hold; the pool binds near 1,387 rps.
#
# Start a variant on the app server yourself first, then run this here:
#
#   ./run-db-heavy.sh              # the whole ladder
#   ./run-db-heavy.sh 1000         # one rate
#
...
[ -f db-heavy.js ] || { echo "db-heavy.js not found in $PWD"; exit 1; }
...
echo "workload: db-heavy"
echo "rates:    $RATES   x $REPS reps"
echo

for rate in $RATES; do
    for rep in $(seq 1 "$REPS"); do
        tag="${VARIANT}__db-heavy__${rate}rps__r${rep}"
        echo "--- $tag"
        ...
        BASE_URL="$BASE" RATE="$rate" DURATION="$DURATION" WARMUP="$WARMUP" OUT="$tag.json" k6 run --quiet --no-color db-heavy.js 2>&1 | grep -vE 'level=(warning|error)' | sed 's/^/    /'
        if [ -s "$tag.json" ]; then
