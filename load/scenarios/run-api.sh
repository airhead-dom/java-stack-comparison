...
RATES="${1:-250 500 1000 1500 2000}"
REPS="${REPS:-3}"
...
for rate in $RATES; do
    for rep in $(seq 1 "$REPS"); do
        ...
        BASE_URL="$BASE" RATE="$rate" DURATION="$DURATION" WARMUP="$WARMUP" OUT="$OUTDIR/$tag.json" k6 run --quiet --no-color api.js 2>&1 | grep -vE 'level=(warning|error)' | sed 's/^/    /'
