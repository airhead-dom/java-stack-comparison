...
[ -f db.js ] || { echo "db.js not found in $PWD"; exit 1; }
...
for rate in $RATES; do
    for rep in $(seq 1 "$REPS"); do
        ...
        BASE_URL="$BASE" RATE="$rate" DURATION="$DURATION" WARMUP="$WARMUP" OUT="$tag.json" k6 run --quiet --no-color db.js 2>&1 | grep -vE 'level=(warning|error)' | sed 's/^/    /'
        if [ -s "$tag.json" ]; then
