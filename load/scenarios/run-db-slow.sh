#!/usr/bin/env bash
#
# Runs the db-slow rate ladder against whatever variant is currently running on the
# SUT. 100ms connection hold; the pool binds near 200 rps.
#
# Start a variant on the app server yourself first, then run this here:
#
#   ./run-db-slow.sh              # the whole ladder
#   ./run-db-slow.sh 1000         # one rate
#
...
RATES="${1:-50 100 150 200 300 400}"
REPS="${REPS:-3}"
...
[ -f db-slow.js ] || { echo "db-slow.js not found in $PWD"; exit 1; }
...
echo "workload: db-slow"
echo "rates:    $RATES   x $REPS reps"
echo

for rate in $RATES; do
    for rep in $(seq 1 "$REPS"); do
        tag="${VARIANT}__db-slow__${rate}rps__r${rep}"
        echo "--- $tag"
        ...
        BASE_URL="$BASE" RATE="$rate" DURATION="$DURATION" WARMUP="$WARMUP" OUT="$tag.json" k6 run --quiet --no-color db-slow.js 2>&1 | grep -vE 'level=(warning|error)' | sed 's/^/    /'
        if [ -s "$tag.json" ]; then
