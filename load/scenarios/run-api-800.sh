#!/usr/bin/env bash
#
# Runs the api-800 rate ladder against whatever variant is currently running on
# the SUT. One 800ms upstream call and no database, so the scarce resource is
# requests in flight rather than threads: ~800 of them at 1,000 rps.
#
# Start a variant on the app server yourself first, then run this here:
#
#   ./run-api-800.sh              # the whole ladder
#   ./run-api-800.sh 1000         # one rate
#
# Only mvc-virtual and webflux-r2dbc are worth running. 200 Tomcat threads over
# an 800ms hold cap mvc-platform at 250 rps, below the bottom rung.
#
...
WORKLOAD="api-800"
SCRIPT="api-800.js"
LATENCY_S="0.810"                         # healthy latency, for the VU estimate
TIMEOUT_S="2.0"                           # must match timeoutMs in $SCRIPT
...
# in flight for the whole delay - ~1,600 at the top rung - so the limit binds
